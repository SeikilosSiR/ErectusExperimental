...
{
	//toUTF8
	std::string tmp_label = Utils::strtowstr(fmt);
	fmt = tmp_label.c_str();
...
{
	//toUTF8
	std::string tmp_label = Utils::strtowstr(label);
	label = tmp_label.c_str();
...
{
	//toUTF8
	///std::string tmp_label = Utils::strtowstr(preview_value);
	//preview_value = tmp_label.c_str();
...
{
	//toUTF8
	std::string tmp_label = Utils::strtowstr(format);
	format = tmp_label.c_str();
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
	//toUTF8
	std::string tmp_label = Utils::strtowstr(label);
	const char* text = tmp_label.c_str();
	...
	ImGuiStyle& style = g.Style;
	ImVec2 pos = window->DC.CursorPos;
	ImVec2 label_size = CalcTextSize(text, NULL, true);
